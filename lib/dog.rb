class Dog
    ...
    def save
        if self.id 
          ...
          sql = <<-SQL
            INSERT INTO #{Dog.table_name} (name, breed) 
            VALUES (?, ?)
          SQL
    ...
    def self.find_or_create_by(name:, breed:)
        dog = DB[:conn].execute("SELECT * FROM #{Dog.table_name} WHERE name = ? AND breed = ?", name, breed)
        if !dog.empty?
          dog_data = dog[0]
    ...
    def update
        sql = "UPDATE #{Dog.table_name} SET name = ?, breed = ?, id = ?"
